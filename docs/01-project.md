...
2026년 8월 8일 기준 핵심 로드맵 `001~047`, 학습 경험 `048~059`, UI 현대화 `060~067`, 보조 MVP 단위
`011-A`, `011-B`와 APS Schedule Control Tower가 구현되어 있습니다. 표준 Gradle 테스트는
306개 중 287개 통과, 실패 0개, 조건부 19개 스킵이며 별도 성능 테스트 2개가 통과했습니다.
...
| 스케줄링 | 대체 설비 순방향 배정, Changeover·Maintenance, 규칙 선택, Frozen Horizon 재배치 |
| 결과 | ScheduleRun 원본 추적·규칙·KPI 스냅샷, 가공·Changeover·대기시간별 계획 Lead Time |
| 제조 제약 | 방향성 Changeover Time과 특정 날짜의 계획 정비시간 |
| 화면 | 오더 검색·페이지, 범위 간트, 납기 지연, 병목 후보, 기준정보 등록, APS 사용자 가이드 |
| APS 학습 | 과정 A~F, 12개 결정론적 실습, 규칙·제약·재계획 비교, 진도 복원과 결과 코치 |
...
| CI | push·pull request Java 21 표준·성능 Gradle 테스트와 결과 artifact |
| 성능 | 소·중·대 ForwardScheduler와 600오더 학습 기준선, JFR 프로파일링과 빈 비가용 구간 빠른 경로 |
| 데이터 입력 | UTF-8 CSV 검증·정규화 미리보기, 요청 키 기반 원자적 반영·행별 이력·중단 재시도 |
| 실행 운영 | 단일 내부 작업자 큐, 실행 상태·실패 이력 조회, 재시작 복구, 실행·입력·작업·실패 메트릭과 구조화 로그 |

운영 기반 `034~039`, APS 엔진 고도화 `040~043`, 데이터 처리·실행 운영 `044~047`,
APS 학습 경험 `048~059`까지 정의된 로드맵을 완료했습니다. 이후 기능은 실제 사용·측정 결과와
별도 요구사항을 근거로 새 커밋 단위를 정의한 뒤 진행합니다.
...
001 단계에서 확인한 초기 문서 차이와 현재 확정 방향은 다음과 같습니다. 현재 README와 커밋 로드맵은
아래 확정 방향에 맞게 현행화되어 있습니다.

| 항목 | 초기 README | 현재 확정 방향 |
| --- | --- | --- |
...
| 진행상태 | 포괄적인 `In Progress`만 표시 | 핵심 로드맵 완료 표시와 번호별 체크박스·원격 push 이력으로 추적 |
