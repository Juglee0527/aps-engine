...
| `machine` | Machine | 기존 계약과 현재 스케줄러가 사용하는 주 설비 |
...
제거하지 않고 후보로 보존하되 실제 계획 후보에서는 제외합니다. 040에서는 모델과 조회 계약만
추가하고 현재 스케줄러는 계속 주 설비를 사용합니다. 가장 이른 완료시각, 우선순위와 설비 ID를
적용하는 결정론적 선택은 041의 책임입니다.
