...
> 현행 상태: 2026-08-08 기준 `001~047`, `011-A`, `011-B`, APS 학습 경험 `048~059`, UI 현대화 `060~075` 완료
...
핵심 로드맵 이후의 작업은 실제 사용·측정 결과 또는 사용자의 명시적인 요구사항을 근거로 새 번호를
정의합니다. 기존 제외 범위는 별도 요구사항과 문서 갱신 없이 구현하지 않습니다.
...
```text
프로젝트를 기능 데모에서 APS 학습용 애플리케이션으로 확장하기 위한 교육 목표와 학습 원칙을 정의해 주세요.
APS·ERP·MRP·MES의 역할, 단계별 커리큘럼, 결정론적 실습 시나리오와 결과 해설 계약을 문서화해 주세요.
README와 로드맵에서 048~059의 범위와 현재 상태를 추적할 수 있게 갱신해 주세요.
이 단계에서는 실행 코드를 변경하지 마세요.
...
번들러는 추가하지 않고 Spring Boot 정적 리소스에서 직접 로딩하며 기존 기능과 화면 동작을 유지해 주세요.
정적 리소스 진입점과 핵심 모듈 로딩 계약을 테스트해 주세요.
권장 커밋 메시지: refactor: modularize frontend application
```

### [x] 050. 학습 가이드 정보구조 개편

```text
사용자 가이드를 APS 소개, 학습 과정, 시나리오 실습과 결과 읽기의 단계형 구조로 개편해 주세요.
각 학습 단위에 개념 설명, 예상 질문, 실행, 관찰과 해설 영역을 제공하고 기존 첫 계획 샘플 흐름을 유지해 주세요.
반응형 화면과 키보드 접근성을 함께 검증해 주세요.
권장 커밋 메시지: feat: redesign aps learning guide
```

### [x] 051. 학습 시나리오 모델

```text
서버가 결정론적 시나리오 정의를 조회하고 상대 날짜 기반 인스턴스를 생성하도록 학습 시나리오 모델을 구현해 주세요.
생성 데이터 추적, 반복 생성 충돌 방지, 인스턴스 상태와 해당 데이터만 대상으로 하는 초기화 정책을 포함해 주세요.
학습 편의 책임은 핵심 스케줄링 도메인과 분리하고 정상·중복·롤백 경계를 테스트해 주세요.
권장 커밋 메시지: feat: add learning scenario model
```

### [x] 052. 학습 시나리오 계획 범위

```text
학습 시나리오에 속한 생산오더만 선택해 스케줄링할 수 있도록 명시적인 계획 범위를 추가해 주세요.
기존 전체 CONFIRMED 오더 실행 계약은 호환되게 유지하고 다른 시나리오와 사용자 오더가 섞이지 않게 해 주세요.
빈 범위, 다른 상태의 오더, 중복 오더 ID와 대량 범위를 테스트해 주세요.
권장 커밋 메시지: feat: scope schedules to learning scenarios
```

### [x] 053. 기초 APS 학습 시나리오

```text
첫 생산계획, 유한 CAPA, 공정 선후관계와 납기 지연을 학습하는 소규모 결정론적 시나리오를 구현해 주세요.
각 시나리오에 목적, 실행 전 예상, 관찰 포인트, 결과 이유와 후속 실험을 제공해 주세요.
예상 설비 선택, 작업 순서, 지연 여부와 KPI를 자동 테스트로 고정해 주세요.
...
비교만으로 ScheduleRun을 저장하거나 오더 상태를 변경하지 말고 사용자가 선택한 규칙만 기존 실행 경로로 확정하게 해 주세요.
규칙별 차이, 동률 결정론, 원본 무변경과 비교 화면을 테스트해 주세요.
...
제약 적용 전후의 작업 시각, 설비 선택, 대기와 KPI 변화를 비교할 수 있게 해 주세요.
각 제약이 의도된 결과를 재현하고 서로 다른 시나리오에 누출되지 않는지 테스트해 주세요.
...
고정, 이동, 제외와 신규 작업을 구분하고 재계획 전후 KPI와 변경 이유를 설명해 주세요.
경계와 겹친 작업, 취소 오더와 신규 오더를 포함한 기존 재스케줄링 계약을 학습 흐름에서 검증해 주세요.
...
첫 100건 고정 조회를 제거하고 서버 페이지네이션, 검색·필터, 설비·기간별 간트 범위 선택을 구현해 주세요.
대량 작업 전체를 무조건 DOM에 그리지 말고 요약 우선 탐색과 렌더링 상한을 적용해 성능 기준을 검증해 주세요.
...
서버 계정 없이 현재 브라우저에서 진도를 안전하게 복원하고 데이터 상태와 불일치하면 다시 확인할 수 있게 해 주세요.
진도 저장 실패가 핵심 APS 기능을 막지 않게 하고 결과 해설 매핑을 테스트해 주세요.
...
표준 테스트와 별도 성능 기준을 실행하고 사용자 가이드, API, 도메인, README와 로드맵을 실제 동작에 맞게 갱신해 주세요.
남은 제한사항과 다음 학습 확장 후보를 명시하고 핵심 학습 흐름에 회귀가 없는지 확인해 주세요.
권장 커밋 메시지: test: verify aps learning experience
```
...
### [x] 064. 생산오더와 마스터 데이터 작업 흐름

### [x] 065. APS 사용자 가이드 재설계

### [x] 066. 비동기 상태, 접근성과 마이크로 인터랙션

### [x] 067. 통합 시각 검증과 문서 현행화

### [x] 068. 정적 리소스 버전과 렌더링 안전장치

### [x] 069. 앱 셸과 화면 골격 2차 재설계

### [x] 070. 스케줄 Control Tower 정보구조 재설계

### [x] 071. 간트 시간축과 범위 탐색 재작성

### [x] 072. 간트 필터와 작업 상세 드로어

### [x] 073. 타이포그래피와 의미 색상 정립

### [x] 074. 반응형 간트와 모바일 작업 목록

### [x] 075. UI 구조 분리, 통합 검증과 문서 현행화
