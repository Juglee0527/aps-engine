...
서버가 지원하는 실습의 키, 과정, 설명, 실행 전 질문, 관찰 포인트, 결과 해설, 후속 실험과
예상 설비·품목·오더 수를 반환합니다. 기초 과정은 다음 네 시나리오를 제공합니다.

| 키 | 학습 초점 | 데이터 |
| --- | --- | --- |
| `FIRST_PLAN` | 기준정보부터 첫 유한능력 계획까지 | 2설비·2품목·4오더 |
...
`DELETE`는 인스턴스가 추적한 데이터와 완료된 실행 결과만 외래 키 안전 순서로 제거하고 상태를 `RESET`으로 바꿉니다.
이미 초기화한 인스턴스에 다시 요청해도 같은 결과를 반환합니다. 인스턴스 생성은 시나리오별
공장·라인·설비·평일 근무 캘린더·품목·Routing·확정 생산오더를 한 트랜잭션으로 만들며, 중간에
실패하면 인스턴스와 업무 데이터를 모두 롤백합니다.
